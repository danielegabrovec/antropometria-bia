...
ipcMain.handle('abia:load-library', async () => {
  const dir = await ensureDir()
  const file = join(dir, 'library.json')
  if (!existsSync(file)) return { patients: [], visits: [], draft: null }
  const raw = await readFile(file, 'utf8')
  return JSON.parse(raw)
})

ipcMain.handle('abia:save-library', async (_e, payload: unknown) => {
  const dir = await ensureDir()
...
ipcMain.handle('abia:import-file', async () => {
  const win = parentWindow()
  const dialogOpts = {
    properties: ['openFile' as const],
    filters: [
      { name: 'Archivio Antropometria BIA', extensions: ['json'] },
